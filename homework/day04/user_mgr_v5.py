...
users = {}
# 初始化列表标题
title = ('Id', 'Name', 'Age', 'Tel', 'Pass Word')
format_str = '|{0[0]:^10d}|{0[1][name]:^20s}|{0[1][age]:^10d}|{0[1][tel]:^24s}|{1:^24s}|'
format_title = '|{0[0]:^10s}|{0[1]:^20s}|{0[2]:^10s}|{0[3]:^24s}|{0[4]:^24s}|'
# 将title元组直接进行格式转换
...
def save_users():
    ...
    for key, value in users.items():
        user = str(key) + ',' + value['name'] + ',' + str(value['age']) + ',' + value['tel'] + ',' + value['pw'] + '\n'
...
def read_users():
    ...
    for line in read_file:
        user = line.split(',')
        users[int(user[0])] = {'name': user[1], 'age': int(user[2]), 'tel': user[3], 'pw': user[4].strip()}
...
def print_user_list(print_list):
    ...
    if len(print_list) > 0:
        ...
        for user in print_list.items():
            print(format_str.format(user, '*' * len(user[1]['pw'])))
...
def login():
    ...
    while input_cnt <= 3:
        ...
        user_pw = input('请输入用密码：')
        for k, v in users.items():
            if user_name == v['name'] and user_pw == v['pw']:
                login_flag = True
                break
        if login_flag:
            break
