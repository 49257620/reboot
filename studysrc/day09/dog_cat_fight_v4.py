...
class GameAnimal(object):
    ...
    def drop_blood(self, drop):
        self.blood -= drop
        print('[' + self.icon + ']', self.name, '损失了', drop, '血量，剩余', self.blood)

    def attack(self, rival):
        print('[' + self.icon + ']', self.name, '发起了攻击：', end='')
...
class Dog(GameAnimal):
    ...
    def attack(self, rival):
        print('[' + self.icon + ']', self.name, '发起了攻击：', end='')
        rival.drop_blood(random.randint(5, 30))


class Cat(GameAnimal):
    def __init__(self, name, blood=100):
        # super().__init__(name,blood,'Cat')  #python3
