...
def MakeFigure(firsts, others):
    ...
    myplot.Save(root='pdf/conditional',
                xlabel='weeks',
                ylabel=r'Prob{x $=$ weeks | x $\geq$ weeks}',
                title='Conditional Probability')


def MakeFigurePmf(firsts, others):
    """Makes a figure showing...
    """
    ...
    # probs is a map from table name to list of conditional probabilities
    probs = {}
    for pmf in [firsts, others]:
        name = pmf.name
        probs[name] = []
        for week in weeks:
            cond = ConditionOnWeeks(pmf, week)
            prob = cond.Prob(week)
...
def RelativeRisk(first, others, week=39):
...
def main():
    ...
    #print(first_cond.Items(),other_cond.Items())
