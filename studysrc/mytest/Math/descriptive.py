...
def MakeTables(data_dir='./data_dir'):
...
def Summarize(pool, firsts, others):
    ...
    live_lengths_list = (list(live_lengths))
    live_lengths_list.sort(key=lambda x: x[0])
    print('Shortest lengths:')
    for weeks, count in live_lengths_list[:10]:
        print(weeks, count)
...
def MakeFigures(firsts, others):
    ...
    myplot.Save(root='pdf/nsfg_hist',
                title='Histogram',
                xlabel='weeks',
                ylabel='frequency',
                axis=axis)
...
def MakeDiffFigure(firsts, others):
    ...
    myplot.Save(root='pdf/nsfg_diffs',
